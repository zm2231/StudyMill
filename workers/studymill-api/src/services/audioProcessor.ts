...
Provide only the JSON array. No explanatory text or markdown formatting.`;
    ...
    try {
      ...
      const generatedText = await genAI.generateText(
        prompt,
        'You are an expert at segmenting academic lectures. Output JSON only.',
        { temperature: 0.1, topK: 32, topP: 0.9, maxOutputTokens: 4096 }
      );

      if (!generatedText) {
        throw new Error('No content generated by Gemini SDK');
      }
