...
import { SemesterPicker } from '@/components/courses/SemesterPicker';
...
export default function CourseManagementPage() {
  ...
  const fetchCourses = async () => {
    ...
    try {
      ...
      const data = await api.request<{ courses: CourseWithDetails[] }>(url);
  ...
  const handleArchiveCourse = async (courseId: string) => {
    try {
      const { api } = await import('@/lib/api');
      await api.request(`/api/v1/courses/${courseId}`, {
        method: 'PATCH',
        body: { archived: true }
      });
  ...
  return (
    <Container size="xl" py="md">
      ...
      {/* Edit Course Modal - can reuse CourseCreation with props */}
      <CourseCreation
